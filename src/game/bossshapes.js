...
//
// TWO HULLS, TWO VEHICLES — was eight and five, and briefly four and three.
// The combat drone and the cargo drone keep their one apiece; the armoured rig,
// the hovercraft and the fighter plane have none left (see below). `group` says
// which vehicle a hull belongs to, so the gallery can show a vehicle's options
// side by side and the boss session can find them.
//
// EIGHT ARE GONE FROM THIS LIST, and that is this file working as intended
...
// fourth. The ROAD TRAIN followed, and NOT to a boss record — it graduated
...
// entry claims it, boss or not. The two FIGHTER PLANE hulls went together and
// last, the day cartypes.js grew the FIGHTER and the MANTA and behaviours.js
// grew the `flyover` tactic they were waiting on — the only group that ever
// ARRIVED here rather than leaving, and the only pair to graduate at once. All
// eight are in carshapes.js now. See the notes where each group used to be, and
// expect this count to keep falling as the rest are claimed.
//
