...
const CAR_FIELD_DESCRIPTIONS = {
  ...
  speedMax: "HARD BAND, top. World units/sec above which nothing may drive this car, whatever it is trying to do — passing, holding station over a mine drop, fleeing. Equal to the cruise top on every car as shipped, which is why passEffort does nothing until you open a gap here. Chasing has its own ceiling: chaseSpeed, on the driving profile.",
  ...
  chaseSpeed: "The chase ceiling in world units/sec — an ABSOLUTE speed, not a multiple of anything. Deliberately allowed above the car's own speedMax: the cruise band governs ordinary driving, this is what the car spends once the player is worth chasing. It sits on the shared driving profile, so it moves every car on that profile — to give ONE car headroom, raise its speedMax in the Speed group instead.",
