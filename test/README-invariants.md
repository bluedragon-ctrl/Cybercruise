...
| `present.test.js` | the GPU present path's failure behaviour (no WebGL2, a `getContext` that throws) — WebGL2 is required as of Phase 15d-i, so this is what asserts the game fails with a message rather than half-built, something no machine with WebGL2 would otherwise exercise — and the sampler name shared between `present.js` and the shader it binds |
